...
export const Login = () => {
  ...
  const removeForm = contextPop.setPopUp;
  const removeState = contextPop.popUP;
  ...
  async function Submit(e) {
    ...
    try {
      const res = await axios.post(
        "https://add-likes.onrender.com/add4like/api/v1/auth/login",
        { email: email, password: password }
      );
      removeForm(!removeState);
      setPassword("");
      setEmail("");
      navigate("/website");
      console.log(res);
      const token = res.data.token;
      Cookie.set("addLike", token);
    } catch (err) {
      console.log(err);
    }
  }
